...
const PerformanceChart = ({ chartData, metrics }) => {
  const { loading, loadingText, startLoading, stopLoading } = useLoader(); // Use loader hook
  const [selectedMetrics, setSelectedMetrics] = useState([]);
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const categories = chartData?.result?.categories || [];
  const series = chartData?.result?.series || [];
  ...
  // Use the loader when data is being processed or fetched
  useEffect(() => {
    if (!chartData) {
      startLoading("Loading dashboard data..."); // Start loader with custom text
    } else {
      stopLoading(); // Stop loader once the data is ready
    }
  }, [chartData, startLoading, stopLoading]);
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <Typography variant="h6" style={{ margin: 0 }}>Performance Overview</Typography>
          <p style={{ margin: 0, color: "#888" }}>
            Key Metrics For Dayparting schedule Performance Evaluation
          </p>
        </div>
